...
### __요약:__
1. 스케줄러 구현: 프로세스 구조체에 우선순위 변수(priority)를 추가해 스케줄러(scheduler)를 구현한다.  
2. starvation 현상 확인: 구현한 스케줄러를 통해 우선순위가 낮은 프로세스들에 starvation 현상이 나타나는 것을 확인한다.
3. aging 변수 추가: starvation 현상 방지를 위해 대기 시간을 age 변수에 기록하고 100초과 시 해당 프로세스가 바로 실행될 수 있게 하였다.
